...
public class RobotLayoutUtil {
	...
	public void transferLayoutInformationToModel(org.eclipse.emf.ecore.EObject element) {
		...
		for (java.util.Iterator<robotG.resource.robot.mopp.RobotLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			robotG.resource.robot.mopp.RobotLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(org.eclipse.emf.ecore.EObject element) {
		...
		if (layoutReference != null) {
			...
			for (java.util.Iterator<org.eclipse.emf.ecore.EObject> i = list.iterator(); i.hasNext(); ) {
				...
				robotG.resource.robot.mopp.RobotLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public org.eclipse.emf.ecore.EObject createLayoutInformationModelElement(robotG.resource.robot.mopp.RobotLayoutInformation layoutInformation, org.eclipse.emf.ecore.EPackage layoutPackage) {
		...
		robotG.resource.robot.grammar.RobotSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), robotG.resource.robot.grammar.RobotGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public robotG.resource.robot.mopp.RobotLayoutInformation createLayoutInformation(org.eclipse.emf.ecore.EObject layoutInformationModelElement) {
		Object object = null;
		...
		robotG.resource.robot.grammar.RobotSyntaxElement syntaxElement = robotG.resource.robot.grammar.RobotGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof robotG.resource.robot.grammar.RobotPlaceholder) {
			robotG.resource.robot.grammar.RobotPlaceholder placeholder = (robotG.resource.robot.grammar.RobotPlaceholder) syntaxElement;
			robotG.resource.robot.IRobotTokenResolver tokenResolver = new robotG.resource.robot.mopp.RobotTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			robotG.resource.robot.IRobotTokenResolveResult result = new robotG.resource.robot.mopp.RobotTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new robotG.resource.robot.mopp.RobotLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public robotG.resource.robot.mopp.RobotLayoutInformationAdapter getLayoutInformationAdapter(org.eclipse.emf.ecore.EObject element) {
