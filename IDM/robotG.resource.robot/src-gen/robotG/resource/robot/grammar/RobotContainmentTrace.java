...
public class RobotContainmentTrace {
	 ...
	 */
	private robotG.resource.robot.mopp.RobotContainedFeature[] path;
	
	public RobotContainmentTrace(org.eclipse.emf.ecore.EClass startClass, robotG.resource.robot.mopp.RobotContainedFeature[] path) {
		super();
		// Verify arguments
		if (startClass != null) {
			if (path.length > 0) {
	...
	public robotG.resource.robot.mopp.RobotContainedFeature[] getPath() {
		return path;
	}
	
	public String toString() {
		return (startClass == null ? "null" : startClass.getName()) + "->" + robotG.resource.robot.util.RobotStringUtil.explode(path, "->");
	}
