...
public class RobotResourceFactoryDelegator implements org.eclipse.emf.ecore.resource.Resource.Factory {
	
	protected java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories = null;
	
	public RobotResourceFactoryDelegator() {
		init();
	}
	...
	protected void init() {
		...
		if (new robotG.resource.robot.util.RobotRuntimeUtil().isEclipsePlatformAvailable()) {
			new robotG.resource.robot.util.RobotEclipseProxy().getResourceFactoryExtensions(factories);
		}
		if (factories.get("") == null) {
			factories.put("", new robotG.resource.robot.mopp.RobotResourceFactory());
		}
	}
