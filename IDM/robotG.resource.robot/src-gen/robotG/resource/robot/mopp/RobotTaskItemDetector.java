...
/**
 * The RobotTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * RobotTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class RobotTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<robotG.resource.robot.mopp.RobotTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<robotG.resource.robot.mopp.RobotTaskItem> foundItems = new java.util.ArrayList<robotG.resource.robot.mopp.RobotTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new robotG.resource.robot.mopp.RobotTaskItem(keyword, message, localLine, offset, end));
