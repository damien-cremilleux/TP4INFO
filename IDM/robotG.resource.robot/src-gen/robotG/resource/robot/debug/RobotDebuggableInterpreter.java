 ...
 */
public class RobotDebuggableInterpreter<ResultType, ContextType> extends robotG.resource.robot.debug.AbstractRobotDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private robotG.resource.robot.util.AbstractRobotInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private robotG.resource.robot.IRobotCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public RobotDebuggableInterpreter(robotG.resource.robot.util.AbstractRobotInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new robotG.resource.robot.IRobotInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = robotG.resource.robot.util.RobotStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public robotG.resource.robot.util.AbstractRobotInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		robotG.resource.robot.IRobotLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private robotG.resource.robot.IRobotLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		robotG.resource.robot.IRobotCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(robotG.resource.robot.debug.ERobotDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = robotG.resource.robot.util.RobotEObjectUtil.getDepth(current);
		stopCondition = new robotG.resource.robot.IRobotCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = robotG.resource.robot.util.RobotEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = robotG.resource.robot.util.RobotEObjectUtil.getDepth(current) - 1;
		stopCondition = new robotG.resource.robot.IRobotCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
