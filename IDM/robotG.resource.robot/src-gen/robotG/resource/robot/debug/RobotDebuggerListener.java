 ...
 * code).
 */
public class RobotDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private robotG.resource.robot.debug.AbstractRobotDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, robotG.resource.robot.util.RobotPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, robotG.resource.robot.util.RobotPair<String, Object>>();
	...
	public RobotDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		robotG.resource.robot.debug.RobotDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = robotG.resource.robot.util.RobotStringUtil.encode('#', stack);
				robotG.resource.robot.debug.RobotDebugMessage message = new robotG.resource.robot.debug.RobotDebugMessage(robotG.resource.robot.debug.ERobotDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				robotG.resource.robot.debug.RobotDebugMessage message = new robotG.resource.robot.debug.RobotDebugMessage(robotG.resource.robot.debug.ERobotDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.GET_VARIABLES)) {
				...
				robotG.resource.robot.debug.RobotDebugMessage message = new robotG.resource.robot.debug.RobotDebugMessage(robotG.resource.robot.debug.ERobotDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return robotG.resource.robot.util.RobotStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		robotG.resource.robot.util.RobotPair<String, Object> pair = new robotG.resource.robot.util.RobotPair<String, Object>(name, value);
	...
	public robotG.resource.robot.debug.AbstractRobotDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(robotG.resource.robot.debug.AbstractRobotDebuggable debuggable) {
