...
public class RobotDebugThread extends robotG.resource.robot.debug.RobotDebugElement implements org.eclipse.debug.core.model.IThread, robotG.resource.robot.debug.IRobotDebugEventListener {
	
	private boolean suspended = false;
	private robotG.resource.robot.debug.RobotDebugTarget debugTarget;
	
	public RobotDebugThread(robotG.resource.robot.debug.RobotDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			robotG.resource.robot.debug.RobotDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = robotG.resource.robot.util.RobotStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new robotG.resource.robot.debug.RobotStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public robotG.resource.robot.debug.RobotDebugTarget getTarget() {
		return debugTarget;
	}
