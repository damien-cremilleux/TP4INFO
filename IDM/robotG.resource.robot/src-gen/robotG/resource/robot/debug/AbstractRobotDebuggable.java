...
public abstract class AbstractRobotDebuggable {
	 ...
	 */
	private java.util.List<robotG.resource.robot.util.RobotPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<robotG.resource.robot.util.RobotPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new robotG.resource.robot.util.RobotRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new robotG.resource.robot.util.RobotRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(robotG.resource.robot.debug.ERobotDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			robotG.resource.robot.debug.RobotDebugMessage message = new robotG.resource.robot.debug.RobotDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				robotG.resource.robot.util.RobotPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(robotG.resource.robot.debug.ERobotDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(robotG.resource.robot.debug.ERobotDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new robotG.resource.robot.util.RobotPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new robotG.resource.robot.util.RobotPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(robotG.resource.robot.debug.ERobotDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(robotG.resource.robot.debug.ERobotDebugMessageTypes.STARTED, true);
			setSuspend(true);
