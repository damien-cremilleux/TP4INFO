...
public class RobotDebugTarget extends robotG.resource.robot.debug.RobotDebugElement implements org.eclipse.debug.core.model.IDebugTarget, robotG.resource.robot.debug.IRobotDebugEventListener {
	
	/**
	 * associated system process
	 */
	private robotG.resource.robot.debug.RobotDebugProcess process;
	...
	private robotG.resource.robot.debug.RobotDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<robotG.resource.robot.debug.IRobotDebugEventListener> eventListeners = new java.util.ArrayList<robotG.resource.robot.debug.IRobotDebugEventListener>();
	
	private robotG.resource.robot.debug.RobotDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new robotG.resource.robot.mopp.RobotMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				robotG.resource.robot.debug.RobotDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(robotG.resource.robot.debug.RobotDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((robotG.resource.robot.debug.IRobotDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public RobotDebugTarget(robotG.resource.robot.debug.RobotDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new robotG.resource.robot.debug.RobotDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new robotG.resource.robot.debug.RobotDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(robotG.resource.robot.debug.IRobotDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(robotG.resource.robot.debug.IRobotDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new robotG.resource.robot.mopp.RobotMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					robotG.resource.robot.debug.RobotLineBreakpoint lineBreakpoint = (robotG.resource.robot.debug.RobotLineBreakpoint) breakpoint;
	...
	public void handleMessage(robotG.resource.robot.debug.RobotDebugMessage message) {
		try {
			if (message.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(robotG.resource.robot.debug.ERobotDebugMessageTypes.RESUMED)) {
	...
	public robotG.resource.robot.debug.RobotDebugProxy getDebugProxy() {
