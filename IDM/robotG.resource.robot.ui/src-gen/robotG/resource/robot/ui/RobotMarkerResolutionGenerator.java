...
public class RobotMarkerResolutionGenerator implements org.eclipse.ui.IMarkerResolutionGenerator {
	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(robotG.resource.robot.IRobotOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof robotG.resource.robot.mopp.RobotResource) {
					robotG.resource.robot.mopp.RobotResource customResource = (robotG.resource.robot.mopp.RobotResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<robotG.resource.robot.IRobotQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final robotG.resource.robot.IRobotQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									robotG.resource.robot.ui.RobotUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
			...
			robotG.resource.robot.ui.RobotUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<robotG.resource.robot.IRobotQuickFix> getQuickFixes(robotG.resource.robot.IRobotTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					robotG.resource.robot.IRobotQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {
