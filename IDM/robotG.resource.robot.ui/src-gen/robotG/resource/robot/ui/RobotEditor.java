 ...
 * <p>
 * This editor has id <code>robotG.resource.robot.ui.RobotEditor</code>
 * The editor's context menu has id
 * <code>robotG.resource.robot.EditorContext</code>.
 * The editor's ruler context menu has id
 * <code>robotG.resource.robot.EditorRuler</code>.
 * </p>
 */
public class RobotEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, robotG.resource.robot.IRobotResourceProvider, robotG.resource.robot.ui.IRobotBracketHandlerProvider, robotG.resource.robot.ui.IRobotAnnotationModelProvider {
	
	private robotG.resource.robot.ui.RobotHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private robotG.resource.robot.ui.RobotCodeFoldingManager codeFoldingManager;
	private robotG.resource.robot.ui.RobotBackgroundParsingStrategy bgParsingStrategy = new robotG.resource.robot.ui.RobotBackgroundParsingStrategy();
	private java.util.Collection<robotG.resource.robot.IRobotBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<robotG.resource.robot.IRobotBackgroundParsingListener>();
	private robotG.resource.robot.ui.RobotColorManager colorManager = new robotG.resource.robot.ui.RobotColorManager();
	private robotG.resource.robot.ui.RobotOutlinePage outlinePage;
	...
	private robotG.resource.robot.ui.RobotPropertySheetPage propertySheetPage;
	...
	private robotG.resource.robot.ui.IRobotBracketHandler bracketHandler;
	...
	public RobotEditor() {
		super();
		setSourceViewerConfiguration(new robotG.resource.robot.ui.RobotSourceViewerConfiguration(this, this, this, colorManager));
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								robotG.resource.robot.IRobotTextResource currentResource = getResource();
				...
				robotG.resource.robot.ui.RobotUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("robotG.resource.robot.EditorContext");
		setRulerContextMenuId("robotG.resource.robot.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new robotG.resource.robot.ui.RobotHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new robotG.resource.robot.ui.RobotCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		robotG.resource.robot.mopp.RobotNature.activate(inputFile.getProject());
		...
		robotG.resource.robot.IRobotTextResource loadedResource = (robotG.resource.robot.IRobotTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				robotG.resource.robot.IRobotTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof robotG.resource.robot.IRobotTextResource) {
					setResource((robotG.resource.robot.IRobotTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					robotG.resource.robot.ui.RobotUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the RobotEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				robotG.resource.robot.ui.RobotUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(robotG.resource.robot.IRobotTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new robotG.resource.robot.ui.RobotOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new robotG.resource.robot.ui.RobotPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			robotG.resource.robot.IRobotTextResource textResource = (robotG.resource.robot.IRobotTextResource) element.eResource();
			robotG.resource.robot.IRobotLocationMap locationMap = textResource.getLocationMap();
			int destination = locationMap.getCharStart(element);
			int length = locationMap.getCharEnd(element) + 1 - destination;
			
			robotG.resource.robot.IRobotTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				robotG.resource.robot.IRobotTextToken token = lexer.getNextToken();
			...
			robotG.resource.robot.ui.RobotUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	public void addBackgroundParsingListener(robotG.resource.robot.IRobotBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (robotG.resource.robot.IRobotBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public robotG.resource.robot.ui.IRobotBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(robotG.resource.robot.ui.IRobotBracketHandler bracketHandler) {
	...
	private boolean setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
		if (selection instanceof org.eclipse.jface.viewers.IStructuredSelection) {
			...
			if (object instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (!(resource instanceof robotG.resource.robot.IRobotTextResource)) {
					return false;
				}
				robotG.resource.robot.IRobotTextResource textResource = (robotG.resource.robot.IRobotTextResource) resource;
				robotG.resource.robot.IRobotLocationMap locationMap = textResource.getLocationMap();
				int destination = locationMap.getCharStart(element);
				if (destination < 0) {
					destination = 0;
