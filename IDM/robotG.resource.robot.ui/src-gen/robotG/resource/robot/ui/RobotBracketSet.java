...
public class RobotBracketSet {
	...
	private java.util.ArrayList<robotG.resource.robot.IRobotBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements robotG.resource.robot.IRobotBracketPair {
	 ...
	 */
	private class ClosingListener implements robotG.resource.robot.ui.IRobotBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				robotG.resource.robot.IRobotBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public RobotBracketSet(robotG.resource.robot.ui.RobotEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new robotG.resource.robot.mopp.RobotMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<robotG.resource.robot.IRobotBracketPair>();
	 ...
	 */
	public robotG.resource.robot.IRobotBracketPair getBracketPair(String opening, String closing) {
		for (robotG.resource.robot.IRobotBracketPair bracketPair : bracketPairs) {
	...
	public robotG.resource.robot.IRobotBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(robotG.resource.robot.IRobotBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public robotG.resource.robot.IRobotBracketPair remove(String opening, String closing) {
		for (robotG.resource.robot.IRobotBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<robotG.resource.robot.IRobotBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, robotG.resource.robot.ui.RobotPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, robotG.resource.robot.ui.RobotPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
