 ...
 */
public class RobotHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static robotG.resource.robot.ui.RobotPositionHelper positionHelper = new robotG.resource.robot.ui.RobotPositionHelper();
	...
	private robotG.resource.robot.ui.RobotTokenScanner scanner;
	...
	private robotG.resource.robot.ui.RobotOccurrence occurrence;
	private robotG.resource.robot.ui.RobotBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, robotG.resource.robot.IRobotBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public RobotHighlighting(robotG.resource.robot.IRobotTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, robotG.resource.robot.ui.RobotColorManager colorManager, robotG.resource.robot.ui.RobotEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = robotG.resource.robot.ui.RobotUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new robotG.resource.robot.ui.RobotTokenScanner(textResource, colorManager);
		occurrence = new robotG.resource.robot.ui.RobotOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new robotG.resource.robot.ui.RobotBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(robotG.resource.robot.ui.RobotEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, robotG.resource.robot.ui.RobotPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, robotG.resource.robot.ui.RobotPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(robotG.resource.robot.ui.RobotPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new robotG.resource.robot.ui.RobotEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
