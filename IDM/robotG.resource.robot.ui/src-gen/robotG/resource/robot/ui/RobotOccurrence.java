 ...
 */
public class RobotOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "robotG.resource.robot.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "robotG.resource.robot.ui.occurences.declaration";
	...
	private robotG.resource.robot.ui.RobotTokenScanner tokenScanner;
	...
	private robotG.resource.robot.IRobotTextResource textResource;
	 ...
	 */
	public RobotOccurrence(robotG.resource.robot.IRobotTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, robotG.resource.robot.ui.RobotTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new robotG.resource.robot.mopp.RobotMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(robotG.resource.robot.ui.RobotBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, robotG.resource.robot.ui.RobotPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, robotG.resource.robot.ui.RobotPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, robotG.resource.robot.ui.RobotPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == robotG.resource.robot.ui.RobotPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(robotG.resource.robot.ui.RobotOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(robotG.resource.robot.ui.RobotOccurrence.DECLARATION_ANNOTATION_ID);
	}
