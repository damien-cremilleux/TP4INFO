...
public class RobotBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	...
	private String BRACKETS_COLOR = robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new robotG.resource.robot.mopp.RobotMetaInformation().getSyntaxName();
	
	private robotG.resource.robot.ui.RobotBracketSet bracketsTmp;
	...
	public RobotBracketPreferencePage() {
		...
		robotG.resource.robot.IRobotMetaInformation metaInformation = new robotG.resource.robot.mopp.RobotMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		...
		bracketsTmp = new robotG.resource.robot.ui.RobotBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					robotG.resource.robot.IRobotBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					robotG.resource.robot.IRobotBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + robotG.resource.robot.ui.RobotPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof robotG.resource.robot.ui.RobotEditor) {
			((robotG.resource.robot.ui.RobotEditor) editor).invalidateTextRepresentation();
		}
	}
